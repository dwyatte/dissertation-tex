...
Behavioral measures were highly variable across objects. Differences between predictability conditions during training appeared to be driven primarily by degenerate viewing angles caused by three-dimensional foreshortening in the objects used. The amount of three-dimensional structure captured by a two-dimensional projection has previously been noted as a principal component of variability in accuracy \cite{BalasSinha09}, but only in a spatially and temporally predictable learning context.

\subsection{}

% increases in RT with spatially coherent stuff \cite{HarmanHumphrey99}
% * no temporal condition
% * increase in RT for objects studied S+. RR was fastest (RR is random presentation, random object identity, so objects get switched over the sequence)
% * no changes in accuracy for S+ (we show a decrease)
...
% degraaf -- predictable @ 10 Hz decreased, but not for recognition task

% previous research on spatial ordering during learning
% * balas dissertation
% * vuong & tarr -- more related to pleast experiment, discuss elsewhere?
% * LawsonHumphreysWatson94 -- familiar objects, S+ acc advantage
...
% * Wallis96 -- Presentation order affects human object recognition learning -- faces, first ``confusion'' as a function of temporal integration study he did

% Li & Dicarlo spatiotemporal learning -- see also bulthoff, wallis
% * Miyashita (1993) and Stryker (1991) have proposed that a mechanism in the temporal cortex that associates successively presented stimuli may play a fundamental role in constructing view-invariant representations of objects.

% limitations of current experiment, how to fix them
%	how to specifically test degenerate view hypothesis
