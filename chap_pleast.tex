...
\chapter{Effects of spatial and temporal predictability during novel object recognition: \\ Temporal and spectral signatures}
...
The effect of spatial predictability manifested 26 ms after the onset of the entrainer and persisted for at least another 24 ms (one quarter of the 10 Hz period). Temporal predictability, in contrast, manifested prior to (-38 through -22 ms pre-stimulus) and at the onset of the entrainer (-6 ms pre-stimulus through 18 ms post-stimulus). The effect of temporal predictability appears to be driven by the anti phase relationship between T- and T+ conditions at these time points. Together, these effects demonstrate differential time courses for spatial and temporal predictability.
...
\end{center}
\caption{Probe-evoked activity}{Grand averages for the probe stimulus, including the preceding 200 ms blank. The asterisk in the interaction plot denotes trending significance at the 5\% level when averaging amplitude within the window defined by the dotted lines.}
\label{fig:probe_tla}
\end{figure}
% revision todo: recreate these figs with y-axis numbers
...
% how is all this different than for entrainers?
...
\end{center}
\caption{Effect of entrainer predictability on alpha power}{Alpha-band power over the entraining sequence. The top row depicts main effects of spatial and temporal predictability on oscillatory power in the 5-20 Hz frequency range. The bottom row depicts 10 Hz only effects of spatial predictability (left), temporal predictability (center), and the interaction between these terms with gray shaded ranges indicating significance while controlling for a maximum false discovery rate (FDR) of 5\%. S-/+ refers to spatially unpredictable and predictable, T-/+ to temporally unpredictable and predictable. Time axes indicate total trial time after the initial fixation cross with 0.4 seconds corresponding to the first entrainer.}
...
\end{center}
\caption{Alpha phase coherence before and after probe}{Alpha-band inter-trial coherence (ITC) 200 ms preceding the probe and 400 ms following. Dotted vertical line indicates probe onset. Time axes indicate total trial time after the initial fixation cross.}
\label{fig:probe_ITC_10Hz}
\end{figure}

\subsection{Predictability effects in delta-theta oscillations}
The effects of spatial and temporal predictability on oscillatory properties during the probe itself were investigated in the delta-theta band, centered around 5 Hz. This frequency was identified based on exploratory analysis and was also motivated by alternative models of sensory prediction \cite[e.g.,]{ArnalGiraud12,GiraudPoeppel12}. 5 Hz power and ITC are plotted in Figures \ref{fig:probe_pow_5Hz}-\ref{fig:probe_ITC_5Hz}.
...
%
% TLA
% * temporal effect occurs only before/at stim. spatial after. effect is found for both entrainers and probe -- separate time courses 
% 		Spatial predictability is a post-stimulus process and might represent a comparison computation between the previous stimulus and the currently presented one. Temporal predictability, in contrast, is a truly anticipatory process, occurring before (and at) the onset of each stimulus. 
% * interaction only trending -- why? more power likely needed. laterality effects also possible. also possible that it has no bearing on behavior with the current paradigm, so it will hover around trend levels
%
% TimeFreq 
% * 2-3 entrainers to establish entrainment: consistent with previous reports \cite{MathewsonFabianiGrattonEtAl10}
% * 10 Hz power Spatial transient, temporal longer, but decays
% 		* perhaps with less entrainment, would see an effect of spatial information on oscillations and might see it in behave results
% * 10 Hz phase (ITC) takes longer to build up but persists into (but not through) blank, at least in the case for temporal info
%		* phase likely accounts for RT effects, ITC follows same pattern, consistent with prev literature (look up)
% * No effect of 10 Hz on probe itself, decays through blank, likely because it is too long. S+T+ bonus can be seen in fig, but not signif except for 5 Hz
%		* Super bonus related to IE since it is the only cross-over interaction (ERP interaction not cross-over)
...
% * Relation of oscillatory stuff to LeabraTI
%	  % Low frequency oscillations seem to capture spatial stuff too, not just temporal stuff although it's hard to know for sure without a better expo
...
% Entrainment studies play an increasingly important role for 245 establishing a causal role of brain oscillations for cognitive pro- 246 cesses. Traditionally, the oscillationÐbehaviour relationship has 247 been established by means of correlation or by contrasting 248 oscillations between experimental conditions showing different 249 behavioural performance. In contrast, entrainment studies attempt 250 to modulate brain oscillations so that consequences for behavioural 251 performance can be studied in a controlled manner.

% we also see it during the blank, so ha 
%%%%% 
