...
%Responses were also split according to whether the probe depicted a view interpolated from the entraining sequence or extrapolated from outside the span of the entraining sequence (Figure \ref{fig:pleast_behave_angle}), allowing inference predictability meaningfully interacted with view generalization. Given that distractors were created from probes and thus shared some features even across viewing angles, both ``same'' and ``different'' probe judgements were included in these groupings. Visual inspection indicated that view interpolation versus extrapolation did not elicit reliable differences in behavioral measures and thus no formal statistical comparisons were made.
%
...
%\end{tabular}
%\end{center}
%\caption{Behavioral effects of predictability on view interpolation and extrapolation.}{Accuracy, \textit{d'}, reaction time, and inverse efficiency as a function of entrainment condition. Error bars depict within-subjects error using the method described in \protect\incite{Cousineau05} adapted for standard error.}
...
% TODO: talk about randy's bottom-up alpha suppression (top-down enhancement) story either here or in general discussion chapter
