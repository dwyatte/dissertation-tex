...
% Entrainers -- 10 Hz pow, ITC signif plots (space and time for both) -- unsurprising
%		interesting that space does not persist at 10 Hz, although time does
%		spatial predictability suppresses 10 Hz pow, ITC% Blank -- 10 Hz ITC for time only, no pow
%
%		temporal ITC slightly longer to develop, but persists into (but not through) blank
%		It does for 5 Hz ITC though, since blank corresponds to 1 delta-theta cycle
%			For discussion: better experiment would use a shorter blank (RANDY!) to get bonus effect of S+T+ for 10 Hz
...
\end{center}
\caption{Effect of entrainer predictability on alpha phase coherence}{Alpha-band inter-trial coherence (ITC) over the entraining sequence. The top row depicts main effects of spatial and temporal predictability on ITC coherence in 5-20 Hz frequency range. The bottom row depicts 10 Hz only effects of spatial predictability (left), temporal predictability (center), and the interaction between these terms with gray shaded ranges indicating significance while controlling for a maximum false discovery rate (FDR) of 5\%. S-/+ refers to spatially unpredictable and predictable, T-/+ to temporally unpredictable and predictable. Time axes indicate total trial time after the initial fixation cross with 0.4 seconds corresponding to the first entrainer.}
...
\end{center}
\caption{Alpha phase coherence before and after probe}{Alpha-band inter-trial coherence (ITC) 200 ms preceding the probe and 400 ms following. Dotted vertical line indicates probe onset. Gray shaded ranges indicating significance while controlling for a maximum false discovery rate (FDR) of 5\%. S-/+ refers to spatially unpredictable and predictable, T-/+ to temporally unpredictable and predictable. Time axes indicate total trial time after the initial fixation cross.}
...
% exploratory 
% 5 Hz
% Entrainers pow effect only (although not space first, then time), no ITC
% Blank -- pow space, time effect
%			ITC time, int* (5 Hz super bonus just starting)
% Probe -- ITC time, int* (5 Hz super bonus)
%		-- pow, time

% 35 Hz int

The effects of spatial and temporal predictability on oscillatory properties during the probe itself were investigated in the delta-theta band, centered around 5 Hz. This frequency was identified based on exploratory analysis and was also motivated by alternative models of sensory prediction \cite[e.g.,]{ArnalGiraud12}. 5 Hz power and ITC are plotted in Figures \ref{fig:probe_pow_5Hz}-\ref{fig:probe_ITC_5Hz}.
...
\end{center}
\caption{Delta-theta power before and after probe}{Delta-theta power 200 ms preceding the probe and 400 ms following. Dotted vertical line indicates probe onset. Gray shaded regions depict significant effects of spatial predictability (left), temporal predictability (center), and the interaction between these terms with gray shaded ranges while controlling for a maximum false discovery rate (FDR) of 5\%. S-/+ refers to spatially unpredictable and predictable, T-/+ to temporally unpredictable and predictable. Time axes indicate total trial time after the initial fixation cross.}
...
\end{center}
\caption{Delta-theta phase coherence before and after probe}{Delta-theta inter-trial coherence (ITC) 200 ms preceding the probe and 400 ms following. Dotted vertical line indicates probe onset. Gray shaded regions depict significant effects of spatial predictability (left), temporal predictability (center), and the interaction between these terms with gray shaded ranges while controlling for a maximum false discovery rate (FDR) of 5\%. S-/+ refers to spatially unpredictable and predictable, T-/+ to temporally unpredictable and predictable. Time axes indicate total trial time after the initial fixation cross.}
