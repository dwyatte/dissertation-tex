...
To determine the effect of learning on the representation of the objects, the pairwise cosine was computed on secondary visual unit minus phase activations across all views of all objects. LeabraTI training produced a representation that captures some similarity across sequential views but each view remained relatively distinct, as would be expected of V2-level representations \cite{KobatakeTanaka94,FreemanSimoncelli11}. The proxy for learning used here strengthens the synapses between secondary visual units and higher-level areas that code increasingly invariant representation. In the model, this higher-level area is a localist output layer which can be considered to performing the same net computation that IT cortex performs using a population code \cite{HungKreimanPoggioEtAl05,LiCoxZoccolanEtAl09}. 
