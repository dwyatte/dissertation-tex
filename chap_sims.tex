...
In LeabraTI, deep neurons are considered to be the receiving units but are only active during the minus phase. This is because their state is only computed once (after the plus phase) and is held constant as an input to superficial neurons during the minus phase. To compute the delta signal, the plus phase state of the microcolumn is used, which in the simplified model used here is the firing rate of superficial neurons:
